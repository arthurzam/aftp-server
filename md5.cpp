...
void md5_init(md5_context *ctx)
{
    ...
    ctx->abcd[1] = /*0xefcdab89*/ T_MASK ^ 0x10325476;
...
void md5_finish(md5_context *ctx, uint8_t digest[16])
{
    ...
    for (i = 0; i < 8; ++i)
        data[i] = (uint8_t)(ctx->count[i >> 2] >> ((i & 3) << 3));
    /* Pad to 56 bytes mod 64. */
    md5_append(ctx, pad, ((55 - (ctx->count[0] >> 3)) & 63) + 1);
    /* Append the length. */
    md5_append(ctx, data, 8);
    for (i = 0; i < 16; ++i)
        digest[i] = (uint8_t)(ctx->abcd[i >> 2] >> ((i & 3) << 3));
}

void md5(const void* data, int nbytes, uint8_t digest[16])
{
    md5_context ctx;
    md5_init(&ctx);
    md5_append(&ctx, (uint8_t*)data, nbytes);
